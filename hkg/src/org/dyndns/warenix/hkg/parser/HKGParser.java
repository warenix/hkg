...
public abstract class HKGParser {
  ...
  public static class PageRequest {
    ...
    /**
     * these forums are not cached statically on hkgolden server, need to use aspx method to list
     * topics
     */
    static String[] cachedForums = {"MB", "JT", "SY", "ED", "PT", "BB", "TR", "CO", "TO", "MU",
        "DC", "ST", "WK", "TS", "RA", "HW", "IN", "SW", "AP", "CA", "FN"};
    ...
    public static boolean isCached(String type) {
  ...
  public static final Pattern mSrcPattern = Pattern.compile("src\\s*=\\s*['\"]([^'\"]+)['\"]");

  public static final Pattern mTitlePattern = Pattern.compile("<title>(.*?)</title>");
  public static final Pattern mThreadUserPattern = Pattern.compile("(.*?)\\(評分: (-?[0-9]+)");
  public static final Pattern mHrefPattern = Pattern
      .compile("<a[^>]+href\\s*=\\s*['\"]([^'\"]+)['\"][^>]*>");
  public static final Pattern mThreadIdPattern = Pattern.compile("message=([0-9]+)");
